...
You have been recently recruited as the Software Engineer at Jumanji Railway Software System. You have a big task at hand. You will be given N(1≤N≤100)
...
Input
The first line will contain an integer N(1≤N≤100)
. For the next N
 lines, ith
 line will describe ith
...
for idx in range(n):
    ...
    trains.append((name, -minutes, idx, line))

for i in range(n):
    best = i
    for j in range(i + 1, n):

        if trains[j] < trains[best]:
            best = j
    if best != i:
